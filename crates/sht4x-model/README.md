...
commands only by the bytes they carry and the frontiers they set.

### Excluded

Every other SHT4x command, heater duty-cycle policy and physics, general-call
reset, clock stretching, ambient physics, autonomous CRC corruption, and any
grade-dependent behavior — `SHT4X-ACC-001` records that accuracy specifies a
reading rather than a step in producing one, and the SHT43 calibration of
`SHT4X-SHT43-CAL-001` is out-of-band certificate data rather than a device
operation. Their absence implies no claim.
...
- Construction at an address no SHT4x is documented at returns
  `UnsupportedAddress`. `SHT4X-I2C-ADDR-001` retains three values; serving
  modeled frames anywhere else would invent a device the sources do not place
  there.
