...
The crate is `no_std`, uses abstract `embedded-hal-async` I2C and delay resources, allocates no memory, and forbids unsafe code. The caller owns those resources, power-up timing, scheduling, retries, and recovery policy. The driver owns the serial and measurement commands' execution waits. Model conformance covers the serial-number read and T/RH measurement at high, medium, and low repeatability; no physical-device claim is made.
