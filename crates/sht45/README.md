...
The package supports an implementation-tested serial-number read for one SHT45-AD1B at 7-bit I2C address `0x44`. It writes command `0x89`, reads six response bytes, validates both CRC-8 values, and returns the transmission-order `u32` serial number.

## Platform support

The crate is `no_std`, uses an abstract `embedded-hal-async` I2C bus, allocates no memory, and forbids unsafe code. The caller owns bus resources, power-up timing, scheduling, retries, and recovery policy. No model-conformance or physical-device claim is made.
