...
- **A power level is not a duration.** `SHT45-HEAT-CMD-001` groups the six
  heater bytes by duration; `SHT45-HEAT-PWR-001` binds each to a power. Both are
  needed to justify the `HeaterPower`/`HeaterDuration` mapping, and the tests
  cannot discriminate a rotated power mapping on their own.
